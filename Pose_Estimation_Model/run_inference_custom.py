...
def get_parser():
    ...
    parser.add_argument("--cad_path", nargs="?", help="Optional path to CAD model for reference-point sampling and visualization")
...
def get_reference_points(cad_path, template_points, cfg):
    if cad_path:
        try:
            import trimesh
            mesh = trimesh.load_mesh(cad_path)
            model_points = mesh.sample(cfg.n_sample_model_point).astype(np.float32) / 1000.0
            return model_points, "cad"
        except Exception as exc:
            print(f"Failed to load CAD model from {cad_path}: {exc}")
            print("Falling back to template points for radius estimation and visualization.")

    if template_points is None or len(template_points) == 0:
        raise ValueError("No CAD model or template points are available for custom inference.")

    template_points = np.asarray(template_points, dtype=np.float32)
    if len(template_points) > cfg.n_sample_model_point:
        sample_idx = np.random.choice(
            np.arange(len(template_points)),
            cfg.n_sample_model_point,
            replace=False,
        )
        template_points = template_points[sample_idx]
    return template_points, "template"


def get_test_data(rgb_path, depth_path, cam_path, seg_path, det_score_thresh, cfg, reference_points):
    dets = []
    ...
    radius = np.max(np.linalg.norm(reference_points, axis=1))
    ...
    ret_dict['model'] = torch.FloatTensor(reference_points).unsqueeze(0).repeat(ninstance, 1, 1).cuda()
    ret_dict['K'] = torch.FloatTensor(K).unsqueeze(0).repeat(ninstance, 1, 1).cuda()
    return ret_dict, whole_image, whole_pts.reshape(-1, 3), all_dets
...
if __name__ == "__main__":
    ...
    template_points = torch.cat(all_tem_pts, dim=1).squeeze(0).detach().cpu().numpy()
    with torch.no_grad():
        all_tem_pts, all_tem_feat = model.feature_extraction.get_obj_feats(all_tem, all_tem_pts, all_tem_choose)

    reference_points, reference_source = get_reference_points(cfg.cad_path, template_points, cfg.test_dataset)
    print(f"=> using {reference_source} reference points")

    print("=> loading input data ...")
    input_data, img, whole_pts, detections = get_test_data(
        cfg.rgb_path, cfg.depth_path, cfg.cam_path, cfg.seg_path,
        cfg.det_score_thresh, cfg.test_dataset, reference_points
    )
    ...
    vis_img = visualize(img, pred_rot[valid_masks], pred_trans[valid_masks], reference_points * 1000, K, save_path)
