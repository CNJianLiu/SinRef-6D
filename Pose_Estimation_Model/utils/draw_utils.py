...
def draw_detections(image, pred_rot, pred_trans, model_points, intrinsics, color=(255, 0, 0)):
    draw_image_bbox = image.copy()
    ...
    # 3D points (randomly sample 512 points)
    choose = np.random.choice(np.arange(len(model_points)), 512)
    pts_3d = model_points[choose].T

    # Draw 3D bounding box
    transformed_bbox_3d = pred_rot @ bbox_3d + pred_trans[:, np.newaxis]
    projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
    draw_image_bbox = draw_3d_bbox(draw_image_bbox, projected_bbox, color)

    # Draw point cloud
    transformed_pts_3d = pred_rot @ pts_3d + pred_trans[:, np.newaxis]
