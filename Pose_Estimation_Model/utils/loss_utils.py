...
def compute_correspondence_loss(
    ...
    for idx, atten in enumerate(atten_list):
        # l1 = CE(atten.transpose(1,2).contiguous(), label1).mean(1)
        # l2 = CE(atten.contiguous(), label2).mean(1)
    ...
    # acc
    # pred_label = torch.max(atten_list[-1], dim=2)[1]
    ...
    # from model.fine_point_matching import Calculate_pose
    # pose_calculator = Calculate_pose() 
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # f_1 = f_1.to(device)
    # f_2 = f_2.to(device)
    # label1 = label1.to(device)
    # R,t = pose_calculator(f_1,f_2,label1)
    # dis_r = torch.mean(torch.norm(R - gt_r, dim=1))
    # dis_t = torch.mean(torch.norm(t - gt_t, dim=1))
    # end_points[loss_str + '_loss_pose_dis'] = dis_r + dis_t

    return end_points,label1
