...
def get_chosen_pixel_feats(img, choose):
    ...
    if len(shape) == 3:
        ...
        B, C, H, W = shape#21 256 224 224
        img = img.reshape(B, C, H*W)
    else:
        assert False

    choose = choose.unsqueeze(1).repeat(1, C, 1) #torch.Size([21, 5000])
...
def aug_fine_pose_noise(gt_r, gt_t,
                std_rots=[15, 10, 5, 1.25, 1],
                max_rot=25,
                sel_std_trans=[0.2, 0.2, 0.2],
                max_trans=0.5):
...
def compute_coarse_Rt(
    ...
    pred_label1 = torch.max(pred_score, dim=2)[1]
    pred_label2 = torch.max(pred_score, dim=1)[1]
    weights1 = (pred_label1>0).float()
    weights2 = (pred_label2>0).float()

    pred_score = pred_score.contiguous()
...
def compute_fine_Rt(
    ...
    # label1 = torch.max(assginment_mat, dim=2)[1]
    # label2 = torch.max(assginment_mat, dim=1)[1]
    ...
    # assginment_mat = assginment_mat * (label1>0).float().unsqueeze(2) * (label2>0).float().unsqueeze(1)
