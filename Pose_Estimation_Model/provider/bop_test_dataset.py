...
class BOPTestset():
    def __init__(self, cfg, eval_dataset_name='ycbv', detetion_path=None):
        ...
        if eval_dataset_name == 'tless':
            ...
            model_path = 'models'
        template_dir = resolve_data_path(cfg.template_dir)
        self.template_folder = os.path.join(template_dir, eval_dataset_name)
    ...
    def get_instance(self, data):
        ...
        if obj_id not in self.obj_idxs:
            return None
        ...
        for obj in self.objects:
            obj_id_key = obj.obj_id
            _, tem_pts, _ = self._get_template(obj)
            all_tem_pts[obj_id_key] = tem_pts
