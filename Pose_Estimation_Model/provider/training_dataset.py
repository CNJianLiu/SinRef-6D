...
class Dataset():
    def __init__(self, cfg, num_img_per_epoch=-1):
        ...
        self.data_paths = [
            os.path.join(self.data_dir,'MegaPose-GSO', 'train_pbr_web'),
            os.path.join(self.data_dir,'MegaPose-ShapeNetCore', 'train_pbr_web')
        ]
        self.model_paths = [
    ...
    def read_data(self, index):
        while True: 
            ...
            dataset_type = path_head.split('/')[6][9:]
            ...
            obj_id = gt['obj_id']
            skip_ids = [1531, 15351, 21159, 32503, 40940, 41957]
            ...
            if obj_id in skip_ids:
                index = (index + 1) % len(self.dataset_paths)
                continue

            camera = json.load(open(os.path.join(self.data_dir, path_head+'.camera.json'), 'rb'))
            K = np.array(camera['cam_K']).reshape(3,3)

            tem1_rgb, tem1_pts ,tem1_pose, tem1_choose  = self._get_template(dataset_type, obj_id, 0)
            if tem1_rgb is None:
                index = (index + 1) % len(self.dataset_paths)
                continue
            ...
            tem_pts = np.concatenate([tem1_pts], axis=0)
            radius = np.max(np.linalg.norm(tem_pts, axis=1))
            flag = np.linalg.norm(target_pts, axis=1) < radius * 1.2
            ...
            tem1_pts = (tem1_pts - tem1_pose[:3, 3]/10000) @ tem1_pose [:3, :3]
            ...
            ret_dict = {
                ...
                'tem1_pose': torch.FloatTensor(tem1_pose),
    ...
    def _get_template(self, type, obj_id, tem_index=1):
        tem_index = random.randrange(50,120)
        tem_index_str = f"{tem_index:03d}"
        if type == 'GSO':
        ...
        rgb_path = os.path.join(file_base, '000'+str(tem_index_str)+'.png')
        
        depth_path = os.path.join(file_base, '000'+str(tem_index_str)+'_depth.png')
        pose_path = os.path.join(str(pose_base) +'.npy')
        mask_path = os.path.join(file_base, '000'+str(tem_index_str)+'_mask.png')
        ...
        choose = mask.astype(np.float32).flatten().nonzero()[0]
        depth = load_im(depth_path).astype(np.float32)

        depth = depth / 10000.0
        # fig_img1 = plt.figure()
        # plt.title(str(obj_id)+'depth') 
        # plt.imshow(depth)
        # plt.show()
        ...
        pose = pose[tem_index][:4, :4]
        pose = pose.reshape(4,4).astype(np.float32)
         
        if not os.path.exists(rgb_path):
            return None, None, None

        return rgb, pts_r, pose, choose
